...
trimmed <- og_df %>% 
  select(observer, classroom, mj_nampa_classrooms, mj_caldwell_classrooms, mj_west_bonner_classrooms, t_classrooms, unlist(q_list, use.names = FALSE), visit_number, date_of_visit)
...
trimmed <- trimmed %>% 
  mutate_at(.vars = vars(-date_of_visit),
            .funs = funs(
    case_when(
      is.na(.) ~ "0",
      . == "" ~ "0",
...
# make data long
trimmed <- gather(trimmed, "question", "performance", 4:63)
...
# take out underscores in questions
trimmed <- trimmed %>% 
...
```{r save_to_rds}
saveRDS(trimmed, file = "Lee_Pesky_Dashboard/prek_fid_checklist_cleaned.rds")
```
...
data_vis_loop(trimmed, "Canyonside Christian School -- Allison Cole") # Example
...
data_vis_loop_all(trimmed, unique(trimmed$classroom)[1:68]) #I use 1:68 because I was suspiscious it was taking so long to finish if I didnt subset the data even though the length is indeed 68. 
```
